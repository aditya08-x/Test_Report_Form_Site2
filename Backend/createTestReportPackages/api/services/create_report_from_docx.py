...
def pdf_to_image_pdf(out_file, img_pdf_path, test_engineer_name, test_engineer_dict, approving_authority):
    ...
    letterhead = get_letterhead("letterheadsite2.pickle")
    newPage = []
    for i, page in enumerate(pages):
        ...
        for pixel in pixmap.keys():
            try:
                if np.array_equal(open_cv_image[pixel[0], pixel[1]], [255, 255, 255]):
                    open_cv_image[pixel[0], pixel[1]] = pixmap[pixel]
...
def get_letterhead(stamp_name):
    ...
    stamp_widht = 0
    if create_stamp:
        pages = convert_from_path(r"C:\Users\aditya.verma\Desktop\reportwala\Letterheadsite2.pdf", 200)
